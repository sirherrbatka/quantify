...
;;; Global Variables ***************
;;; ********************************
(defvar *samples* (make-array 1024 :adjustable t :fill-pointer 0)
  "Samples gathered during profiling.")

(declaim (inline number-of-samples))
(defun number-of-samples ()
  (fill-pointer *samples*))

(declaim (inline push-sample))
(defun push-sample (sample)
  (vector-push-extend sample *samples*))

(defun clear-samples ()
  (setf (fill-pointer *samples*) 0)
  *samples*)
...
;;                        (old-nested-calls nested-calls)
                          (start-time (get-time))
                          (start-cons (get-cons))
                          (fresh-sample (make-function-call-sample :name name))
                          (number-of-samples (number-of-samples))
                          (*stack-trace* (cons fresh-sample *stack-trace*)))
                     ...
                     (multiple-value-prog1
                       ...
                       (let ((delta-time (- (get-time) start-time))
                             ...
                             (delta-cons-exclusive (+ delta-cons
                                                      (- prev-total-cons
                                                         *total-cons*))))
                         (setf (function-call-sample-time fresh-sample) delta-time
                               (function-call-sample-cons fresh-sample) delta-cons)
                         ;; only push the innermost samples
                         (when final-call-p
                           (push-sample *stack-trace*))
                         ;; Calls
                         (incf calls)
                         (incf *total-calls*)
...
;                        (setf inclusive-time (+ delta-time old-time))
                         (incf exclusive-time (+ delta-time delta-time-exclusive))
...
(defun unique-calls ()
  (let ((substacks (make-hash-table :test 'eq :size (length *samples*))))
    (loop
      :for stack :across *samples*
      :do (unless (eq 'stub-function (function-call-sample-name (first stack)))
            (let ((exclusive-cost 0))
              (loop :for substack :on stack
                    :for call := (first substack)
                    :for inclusive-cost := (function-call-sample-time call)
                    :do (progn (unless (gethash substack substacks)
                                 (setf (gethash substack substacks) inclusive-cost))
                               (decf (gethash substack substacks) exclusive-cost)
                               (setf exclusive-cost inclusive-cost))))))
    substacks))

(defgeneric format-flamegraph (output))

(defmethod format-flamegraph ((stream stream))
  (let ((sums (make-hash-table :test 'equal)))
    (maphash (lambda (substack total-cost)
               (let* ((key (mapcar #'function-call-sample-name substack))
                      (cell (gethash key sums (cons 0 0))))
                 (incf (car cell) total-cost)
                 (incf (cdr cell))
                 (setf (gethash key sums) cell)))
             (unique-calls))
...
(defmethod format-flamegraph (output)
  (alexandria:with-output-to-file (stream output)
    (format-flamegraph stream)))
